"""
Master Report Updater Script
Updates outputs/REPORT/bao_cao_giam_sat_song_hong.md and docs/multiyear_shoreline_summary_report.md
with complete 20-season metrics, timeline analysis, embedded trend charts, and geomorphological insights.
"""
...
def main():
    report_path = os.path.join(PROJECT_ROOT, "outputs", "REPORT", "bao_cao_giam_sat_song_hong.md")
    docs_report_path = os.path.join(PROJECT_ROOT, "docs", "multiyear_shoreline_summary_report.md")
    
    # Read metrics table from CSVs
    records = []
    for yr in range(2017, 2027):
        for ssn in ['dry', 'wet']:
            csv_p = os.path.join(PROJECT_ROOT, "outputs", str(yr), f"{yr}_{ssn}", "stats", f"validation_statistics_{yr}_{ssn}.csv")
            if os.path.exists(csv_p):
                ...
                if not df.empty:
                    ...
                    records.append({
                        'Year': yr,
                        'Season': ssn.upper(),
                        'Mean': float(row['Mean_Error_m']),
                        'Median': float(row['Median_Error_m']),
                        'RMSE': float(row['RMSE_m']),
                        'P95': float(row['P95_Error_m']),
                        'Hausdorff': float(row['Hausdorff_m']),
                        'Status': 'SUCCESS'
                    })
                    
    metrics_table_md = "| Năm (Year) | Mùa (Season) | Mean Error (m) | Median Error (m) | RMSE (m) | P95 Error (m) | Hausdorff (m) | Trạng Thái |\n"
    metrics_table_md += "| :---: | :---: | :---: | :---: | :---: | :---: | :---: | :---: |\n"
    for r in records:
        metrics_table_md += f"| {r['Year']} | **{r['Season']}** | {r['Mean']:.2f} | {r['Median']:.2f} | **{r['RMSE']:.2f}** | {r['P95']:.2f} | {r['Hausdorff']:.2f} | {r['Status']} |\n"

    full_report_content = f"""# BÁO CÁO PHÂN TÍCH KHOA HỌC 🛰️🌊
...
2. **Độ chính xác tiệm cận chuẩn xuất bản:** Median Error (P50) toàn chuỗi duy trì dưới $20.89\\text{{m}}$ ($\approx 2\\text{{ pixels}}$), sẵn sàng phục vụ các cơ quan quản lý đê điều, thiên tai và quy hoạch đô thị ven sông Hồng.
"""
    ...
    print(f"[SUCCESS] Updated master report at: {report_path}")
    print(f"[SUCCESS] Updated docs report at: {docs_report_path}")
